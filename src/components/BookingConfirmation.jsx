...
const HOTEL_ADDRESS = "Namangan shahar";
const HOTEL_PHONE = "+998 93 911 95 72 Администратор 24/7";
const HOTEL_EMAIL = "test.test@mail.ru";
const HOTEL_NAME = "Demo Hotel Namangan";
const HOTEL_URL = "https://demo.my-hotels.uz";
const HOTEL_MAP_URL =
  "https://www.google.com/maps/search/?api=1&query=40.995713%2C71.588813";
const PURPLE = "#4c2cac";
...
const getStay = (guest) => {
  const checkIn =
    safeDate(guest?.bookedForAt || guest?.checkInAt) || new Date();
  const checkOut =
    safeDate(guest?.checkoutDueAt || guest?.checkOutAt) ||
    addDays(checkIn, Number(guest?.stayDays || 1));
  ...
  return (
    <section className="booking-confirmation-page">
      <div className="booking-browser-head">
        <span>
          Подтверждение бронирования. Бронирование № {reservationNumber}
        </span>
      </div>
      {children}
...
function BookingConfirmation({ guest }) {
  ...
  const total = Number(
    guest.externalTotalAmount ||
      guest.totalAmount ||
      guest.dailyRate * nights ||
      0,
  );
  const paid = Number(guest.paidAmount || 0);
  const due = Math.max(total - paid, 0);
  const dailyRate = Number(
    guest.dailyRate || (nights ? total / nights : total) || 0,
  );
  const room = guest.room || {};
  const roomCategory = String(room.category || "")
    .trim()
    .toLowerCase();
    ...
    >
      <PageChrome reservationNumber={reservationNumber} page={1}>
        <main className="booking-page-content booking-page-one">
          <header className="booking-hero">
            ...
            <p>
              <b>Дата и время бронирования:</b> {formatDateTime(bookedAt)} (UTC
              +05:00)
            </p>
          </header>
          ...
          <section className="booking-hotel-block">
            ...
            <div className="booking-logo-box">
              <img src={istiqlolHotelLogo} alt="Demo Hotel Namangan" />
            </div>
          </section>
          ...
          <section className="booking-section booking-details-section">
            ...
            <div className="booking-summary-grid">
              <div>
                <span>Заезд</span>
                <b>
                  {formatDate(checkIn)}, {weekday(checkIn)}
                </b>
                <small>
                  после {String(checkIn.getHours()).padStart(2, "0")}:
                  {String(checkIn.getMinutes()).padStart(2, "0")}
                </small>
              </div>
              <div>
                <span>Выезд</span>
                <b>
                  {formatDate(checkOut)}, {weekday(checkOut)}
                </b>
                <small>
                  до {String(checkOut.getHours()).padStart(2, "0")}:
                  {String(checkOut.getMinutes()).padStart(2, "0")}
                </small>
              </div>
              <div>
                <span>Ночей</span>
            ...
            <div className="booking-calendar-links">
              <span>
                ▣ &nbsp;<u>Добавить в календарь Outlook / iCal</u>
              </span>
              <a
          ...
          <section className="booking-customer">
            ...
            <div>
              ...
              <b>
                {fullName}
                {guest.passport ? ` Паспорт: ${guest.passport}` : ""}
                {guest.birthDate
                  ? ` Дата рождения: ${formatShortDate(guest.birthDate)}`
                  : ""}
              </b>
            </div>
          </section>
          ...
          <section className="booking-section booking-price-section">
            ...
            <div className="booking-tariff-row">
              <span>Тариф</span>
              <b>Тариф по умолчанию</b>
            </div>
            <div className="booking-cost-table">
              <div className="booking-cost-head">
                <b>№1: {roomLabel}</b>
                <b>
                  {formatMoney(total)}
                  <small>UZS</small>
              ...
              <div className="booking-cost-line">
              ...
              <div className="booking-total-line">
                <b>Общая стоимость</b>
                <b>{formatMoney(total)} UZS</b>
              </div>
              <div className="booking-payment-line">
                <b>Способ оплаты</b>
                <span>
                  {guest.mainPaymentType === "bank"
                    ? "Банковский перевод"
                    : "Оплата при заселении"}
                </span>
              </div>
              <div className="booking-payment-line">
                <b>Внесена предоплата</b>
                <span>{formatMoney(paid)} UZS</span>
              </div>
              <div className="booking-total-line">
                <b>К оплате гостем</b>
                <span>{formatMoney(due)} UZS</span>
              </div>
            </div>
          </section>
          ...
          <section className="booking-tariff-description">
            ...
            <p>
              <span className="booking-bell">●</span>
              <b>Тариф по умолчанию</b>
            </p>
            <p>
              <span>🍴</span>Завтрак "Шведский стол" &nbsp; включен
            </p>
          </section>
        </main>
      </PageChrome>
      ...
      <PageChrome reservationNumber={reservationNumber} page={2}>
        <main className="booking-page-content booking-page-two">
          <section className="booking-room-title">
            <b>№1</b>
            <h2>{roomLabel}</h2>
          </section>
          <div className="booking-room-facts">
            ...
            <div>
              ...
              <span>
                {guest.guestType === "chetellik" ? "Иностранное" : "Узбекистан"}
              </span>
            </div>
          </div>
                ...
                <div className="booking-calendar-cells">
                  {calendar.map(({ date, charged }) => (
                    <div key={date.toISOString()}>
                      ...
                      {charged ? <b>{formatMoney(dailyRate)}</b> : null}
                    </div>
                  ))}
                </div>
                <div className="booking-calendar-total">
                  {formatMoney(total)}
                </div>
              </div>
            </div>
          ...
          <section className="booking-location">
            ...
            <dl>
              ...
              <dd>{HOTEL_ADDRESS}</dd>
              <dt>Координаты:</dt>
              <dd>40.995713, 71.588813</dd>
              <dt>Как добраться:</dt>
              <dd>2.6 км Афсоналар Вэлли Парк, 2 км Парк имени Бабура</dd>
            </dl>
            <div className="booking-map">
              <img
                src={istiqlolHotelMap}
                alt="Карта расположения Demo Hotel Namangan"
              />
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label="Открыть расположение Demo Hotel в Google Maps"
          ...
          <footer className="booking-purple-footer">
            ...
            <p>Письмо автоматически сформировано Demo Suite</p>
          </footer>
        </main>
      </PageChrome>
    </div>
